...
const _excluded = ["intent", "params", "target"],
  _excluded2 = ["onClick", "href", "target", "replace"],
  _excluded3 = ["onClick", "replace", "state", "target", "toIndex"];
...
const IntentLink = React.forwardRef(function IntentLink2(props, ref) {
  ...
  return /* @__PURE__ */jsxRuntime.jsx("a", _objectSpread(_objectSpread({}, restProps), {}, {
    href,
    onClick,
    ref,
    target
...
function addScope(routerState, scope, scopedState) {
  return scopedState && _objectSpread(_objectSpread({}, routerState), {}, {
    [scope]: scopedState
  });
}
function RouteScope(props) {
  ...
  const scopedRouter = React.useMemo(() => _objectSpread(_objectSpread({}, parent), {}, {
    navigate,
...
function RouterProvider(props) {
  ...
  const navigateUrl = React.useCallback(opts => {
    onNavigate(opts);
  }, [onNavigate]);
...
function matchPath(node, path) {
  ...
  const mergedState = _objectSpread(_objectSpread({}, state), childState || {});
  return node.scope ? {
...
function useRouterState() {
  ...
  const [selectedState, setState] = React.useState(() => selector(state));
  React.useEffect(() => setState(selector(state)), [selector, state]);
