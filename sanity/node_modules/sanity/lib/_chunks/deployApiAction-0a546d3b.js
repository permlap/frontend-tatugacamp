...
var _internal = require('./_internal-ca5bebdb.js');
...
var SchemaBuilder = require('@sanity/schema');
var upperFirst = require('lodash/upperFirst.js');
var logSymbols = require('log-symbols');
var getGraphQLAPIs = require('./getGraphQLAPIs-9c75cb2a.js');
...
require('fs/promises');
require('pretty-ms');
require('@sanity/util/fs');
require('@sanity/export');
require('fs');
require('@sanity/import');
require('lodash/padStart');
require('@sanity/eventsource');
require('rxjs');
require('date-fns');
...
require('lodash/noop.js');
require('@sanity/uuid');
require('open');
require('util');
require('lodash/groupBy.js');
require('url');
require('lodash/sortBy.js');
require('lodash/size.js');
...
var oneline__default = /*#__PURE__*/_interopDefaultLegacy(oneline);
var getIt__default = /*#__PURE__*/_interopDefaultLegacy(getIt);
var uniqBy__default = /*#__PURE__*/_interopDefaultLegacy(uniqBy);
var startCase__default = /*#__PURE__*/_interopDefaultLegacy(startCase);
var SchemaBuilder__default = /*#__PURE__*/_interopDefaultLegacy(SchemaBuilder);
...
function getBaseType(baseSchema, typeName) {
  var _a;
  return SchemaBuilder__default["default"].compile({
...
function extractFromSanitySchema(sanitySchema) {
  ...
  function convertType(type, parent) {
    ...
    return _objectSpread(_objectSpread(_objectSpread({}, props), mapped), original);
  }
  ...
  function buildRawField(field, parentName) {
    return _objectSpread(_objectSpread({}, convertType(field, parentName, {
  ...
  function getReferenceDefinition(def, parent) {
    ...
    return candidates.length === 1 ? _objectSpread({
      type: getTypeName(candidates[0].type.name)
    }, base) : _objectSpread(_objectSpread({}, getUnionDefinition(candidates, def, {
      grandParent: parent
    })), base);
  }
  function getArrayDefinition(def, parent) {
    ...
    return candidates.length === 1 ? _objectSpread(_objectSpread({
      children: getArrayChildDefinition(candidates[0], def)
    }, base), name) : _objectSpread(_objectSpread({
  ...
  function getDocumentDefinition(def) {
    ...
    return _objectSpread(_objectSpread({}, objectDef), {}, {
      fields,
      interfaces: ["Document"]
    });
  }
...
function generateTypeSortings$1(types) {
  const objectTypes = types.filter(isNonUnion).filter(type => type.type === "Object" && !["Block", "Span"].includes(type.name) && !type.interfaces && !builtInTypes$1.includes(type.name));
...
function generateTypeSortings(types) {
  const objectTypes = types.filter(isNonUnion).filter(type => type.type === "Object" && !["Block", "Span"].includes(type.name) && !type.interfaces && !builtInTypes.includes(type.name));
  const documentTypes = types.filter(type => type.name === "Document" || isDocumentType(type));
  const hasFields = type => type.fields.length > 0;
...
async function deployGraphQLApiAction(args, context) {
  ...
  if (usedLegacyFlags.length === 1) {
    throw new Error("The --".concat(usedLegacyFlags[0], " flag is no longer supported. The setting can be configured in sanity.cli.ts (environment variables may be used for dynamic configuration)."));
  } else if (usedLegacyFlags.length > 1) {
    const removedFlags = usedLegacyFlags.map(flag => "--".concat(flag)).join(", ");
    throw new Error("The following flags are no longer supported: ".concat(removedFlags, ". Instead, configure these in sanity.cli.ts (environment variables may be used for dynamic configuration)."));
  }
  const {
    apiClient,
    output,
...
async function getCurrentSchemaProps(client, dataset, tag) {
  try {
    const res = await getUrlHeaders(client.getUrl("/apis/graphql/".concat(dataset, "/").concat(tag)), {
...
function parseCliFlags(args) {
  return yargs__default["default"](helpers.hideBin(args.argv || process.argv).slice(2)).option("generation", {
    type: "string"
...
//# sourceMappingURL=deployApiAction-0a546d3b.js.map
