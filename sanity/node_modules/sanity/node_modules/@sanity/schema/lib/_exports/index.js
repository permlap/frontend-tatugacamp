...
function parsePreview(preview) {
  ...
  if (Array.isArray(select)) {
    return _objectSpread(_objectSpread({}, pick__default["default"](preview, ["prepare", "component"])), {}, {
...
function createFieldsets(typeDef, fields) {
  ...
  const fieldsetsByName = keyBy__default["default"](fieldsets, "name");
...
function createFieldsGroups(typeDef, fields) {
  ...
  if (defaultGroups.length > 1) {
    throw new Error("You currently have ".concat(defaultGroups.length, " default field groups defined for type '").concat(typeDef.name ? startCase__default["default"](typeDef.name) : (_a = typeDef.title) != null ? _a : "", "', but only 1 is supported"));
  }
  const groupsByName = keyBy__default["default"](groups, "name");
...
const ImageType = {
  ...
  extend(subTypeDef, extendMember) {
    ...
    const parsed = Object.assign(pick__default["default"](IMAGE_CORE, OVERRIDABLE_FIELDS$2), subTypeDef, {
      type: IMAGE_CORE,
      title: subTypeDef.title || startCase__default["default"](subTypeDef.name || subTypeDef.type || ""),
      options,
