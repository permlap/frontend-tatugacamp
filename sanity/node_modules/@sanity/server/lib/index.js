...
var viteReact__default = /*#__PURE__*/_interopDefaultLegacy(viteReact);
...
const debug$5 = debugIt__default["default"]("sanity:server");
...
function getDocumentHtml(studioRootPath, props) {
  ...
  const result = server.renderToStaticMarkup(react.createElement(Document, _objectSpread(_objectSpread(_objectSpread({}, defaultProps), props), {}, {
    css
  })));
...
function sanityFaviconsPlugin(_ref3) {
  ...
  async function getFavicons() {
    ...
    cache.favicons = await fs__default$1["default"].readdir(faviconsPath);
    return cache.favicons;
  }
  return {
    ...
    configureServer(viteDevServer) {
      ...
      return () => {
        viteDevServer.middlewares.use(async (req, res, next) => {
          ...
          res.write(await fs__default$1["default"].readFile(path__default["default"].join(faviconsPath, fileName)));
          res.end();
        });
      };
    }
...
async function getViteConfig(options) {
  ...
  const faviconsPath = path__default["default"].resolve(__dirname, "../static/favicons");
...
async function getSanityStudioConfigPath(studioRootPath) {
  ...
  if (availableConfigs.length === 0) {
    ...
    return path__default["default"].resolve(__dirname, "./default-config.js");
  }
...
async function writeSanityRuntime(_ref4) {
  ...
  const relativeConfigLocation = path__default["default"].relative(runtimeDir, studioConfigPath);
...
async function copyDir(srcDir, destDir, skipExisting) {
  await fs__default$1["default"].mkdir(destDir, {
    recursive: true
  });
  for (const file of await tryReadDir(srcDir)) {
    ...
    const stat = await fs__default$1["default"].stat(srcFile);
    if (stat.isDirectory()) {
      await copyDir(srcFile, destFile, skipExisting);
    } else if (skipExisting) {
      await fs__default$1["default"].copyFile(srcFile, destFile, fs.constants.COPYFILE_EXCL).catch(skipIfExistsError);
    } else {
      await fs__default$1["default"].copyFile(srcFile, destFile);
    }
  }
}
async function tryReadDir(dir) {
  try {
    const content = await fs__default$1["default"].readdir(dir);
    return content;
  } catch (err) {
    if (err.code === "ENOENT") {
      return [];
...
async function writeFavicons(basePath, destDir) {
  ...
  await copyDir(path__default["default"].resolve(__dirname, "../static/favicons"), destDir, true);
...
async function writeWebManifest(basePath, destDir) {
  ...
  await fs__default$1["default"].writeFile(path__default["default"].join(destDir, "manifest.webmanifest"), content, "utf8").catch(skipIfExistsError);
}
...
async function startPreviewServer(options) {
  ...
  const startTime = Date.now();
  if (!fs__default["default"].existsSync(path__default["default"].join(root, "index.html"))) {
